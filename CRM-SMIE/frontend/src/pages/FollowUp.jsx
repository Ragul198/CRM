...
import { updateLeadStatus } from "../api/fetchdata.jsx"; // ✅ API import
...
const FollowUp = () => {
  ...
  // Only leads with status "Follow-up"
  ...
  const handleNext = () => {
    ...
    try {
      ...
      if (newStatus === "Opportunity" && !lead.assignedTo) {
        toast.error("Please assign an engineer before changing status to Qualified.");
        return;
      }
  ...
  return (
    <div id="table-width-fixed" className="mt-4">
      ...
      <div>
        ...
        /{" "}
        <Link to="/followup" className="text-[#C2410C]">
          FollowUps
        </Link>
      </div>
      ...
      <div className="mt-4">
        ...
        <p className="text-gray-600 mt-3 ml-1">
          Track and manage leads requiring follow-up actions
        </p>
        ...
        {/* Table */}
        <div className="mt-7 overflow-x-auto w-auto rounded-md bg-white shadow-md">  {/* please Delete this p-3 */}
                ...
                paginatedLeads.map((lead) => (
                  <tr key={lead._id} className="hover:bg-gray-50">
                    <td className="p-3 border-b border-gray-300 text-blue-700">
                       <Link to={`/opportunityView/${lead._id}`}state={{ fromSection: "followup" }}>{lead.name}</Link>
                    </td>
                    ...
                    {/* ✅ Status dropdown with DB update */}
                    <td className="p-3 border-b border-gray-300">
                      ...
                      >
                        {[lead.status, "Quotation", "Converted", "Failed"]
